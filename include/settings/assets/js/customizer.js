jQuery( document ).ready(function($) {
	...
	function toggle_control_activity(control){
		if(control.params.container == this.container || (this.container == "any" && control.params.container)){
			let params =  this.duration >= 0 ? {duration: this.duration} : null;
			control.activated_by_container = this.show;
			if(this.show) {
				control.activate(params);
			}
			else {
				control.deactivate(params);
			}
		}
	}
	$(".wa-customizer-container-control").click(function(){
		let dom_container = $(this).closest("[id^=customize-control-]");
		if(dom_container.length == 0)
			return;
		let container_name = dom_container.attr("id").replace("customize-control-", "");
		let container = wp.customize.control(container_name);
		let section_name = container.section();
		let show = !$(this).is(".open");
		if(show)
			$(this).addClass("open");
		else
			$(this).removeClass("open");
		_.each(wp.customize.section(section_name).controls(), toggle_control_activity.bind({container: container_name, show: show}));
	});
	wp.customize.section.each(function(section) {
		let handler = function(expand){
			_.each(section.controls(), toggle_control_activity.bind({container: "any", show: false, duration: 0}));
			section.expanded.unbind(handler);
		};
		section.expanded.bind(handler);
		_.each(section.controls(), function(control) {
			control.active.bind(function(){
				if(control.activated_by_container === "undefined")
					return;
				if(control.activated_by_container == false && control.active()){
					control.container.stop(true, false);
					toggle_control_activity.call({container: "any", show: false, duration: 0}, control);
				}
			});
		});
	});
	...
	$(".pill_checkbox_control .sortable").sortable({
		...
		sort: function (event, ui){
			let target = $(event.target).closest(".pill_checkbox_control .sortable");
			let control = $(event.originalEvent.target).closest("label.ui-sortable-handle");
			if(control.is(".metaitem")) {
	...
	// Get the values from the checkboxes and add to our hidden field
	let record_blocks_sequence_wrong_order_msg_id = null;
	function skyrocketGetAllPillCheckboxes($element) {
		var inputValues = $element.find('.sortable-pill-checkbox').map(function() {
			return $(this).val() + ":" + ($(this).is(':checked') ? "1" : "0");
		}).toArray();
		let lst = inputValues.length - 1;
		let setting_container = $element.closest(".customize-control.customize-control-pill_checkbox");
		if(setting_container.length > 0 && setting_container.attr("id") == "customize-control-record_blocks_sequence"){
			let header = false;
			let valid_order = true;
			for(let i = 0; i < inputValues.length; i++){
				let block = inputValues[i];
				if(block.indexOf("header_open") >= 0)
					header = true;
				if(block.indexOf("header_close") >= 0)
					header = false;
				if(block.indexOf("similar_records") >= 0 && header){
					valid_order = false;
					break;
				}
			}
			if(valid_order)
				...
				let target = $element.find("> .sortable");
				target.sortable('cancel');
				if(record_blocks_sequence_wrong_order_msg_id)
					clearTimeout(record_blocks_sequence_wrong_order_msg_id);
				wp.customize.notifications.add(
					'record_blocks_sequence_wrong_order',
					new wp.customize.Notification(
						'record_blocks_sequence_wrong_order', {
							dismissible: true,
							message: "Не допустимый порядок блоков. Блок похожих записей не может находиться в заголовке поста",
							type: 'error'
						}
					)
				);
				record_blocks_sequence_wrong_order_msg_id = setTimeout(function(){
					wp.customize.notifications.remove("record_blocks_sequence_wrong_order");
				}, 5000);
			}
		}
		else
	...
	/**
	 * WebArch Toggle Switch Text Custom Control
	 */
	function refresh_toggle_switch_text_control_hidden(){
		let p = $(this).closest(".toggle-switch-text-control");
		let active = p.find("input[type=checkbox]").is(":checked");
		let text = p.find("input[type=text]").val();
		p.find("input[type=hidden]").val(JSON.stringify({act: active, text: text})).trigger("change");
		let ex = p.find(".example");
		ex.html(ex.html().replace(/<b>[^<]*<\/b>/, "<b>" + text + "</b>"));
	}
	$(".toggle-switch-text-control input[type=checkbox], .toggle-switch-text-control input[type=text]").change(refresh_toggle_switch_text_control_hidden);

	/**
	 * WebArch Just Current Page
	 */
	/*let jcp = $("#just_current_page").closest("div.just_current_page");
	jcp.closest("li").hide();
	jcp.appendTo(jcp.closest("form"));
	let freezeContainerClick = false;
	function jcp_on_change(e){
		if(!$(e.currentTarget).is("input")) {
			$(e.currentTarget).find("input").click();
		}
		else{
			if(freezeContainerClick)

			if($(this).prop("checked"))
				$(this).closest(".just_current_page").addClass("checked");
			else
				$(this).closest(".just_current_page").removeClass("checked");
		}
	}
	jcp.click(jcp_on_change);
	jcp.find("input[type=checkbox]").bind("click", jcp_on_change);*/
