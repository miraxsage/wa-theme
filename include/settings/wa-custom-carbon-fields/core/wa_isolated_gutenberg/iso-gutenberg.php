...
class IsoEditor_Gutenberg {
     ...
     * @return array
     */
    public function block_editor_settings_all($settings, $editor_context = null) {
    ...
    public function get_editor_settings() {
            ...
            foreach ( $editor_styles as $style ) {
                if ( preg_match( '~^(https?:)?//~', $style ) ) {
                    ...
                    if ( ! is_wp_error( $response ) ) {
                        $styles[] = array(
                            'css' => $this->normalizeJson(wp_remote_retrieve_body( $response )),
                        );
                    }
                } else {
                    ...
                    if ( is_file( $file ) ) {
                        $styles[] = array(
                            'css'     => $this->normalizeJson(file_get_contents( $file )),
         ...
         */
        $body_placeholder = apply_filters( 'write_your_story', __( 'Введите /, чтобы выбрать блок' ), $post );
         ...
         * @psalm-suppress TooManyArguments
         */
        $res = apply_filters( 'block_editor_settings_all', $editor_settings, $post );
        for($i = 0; $i < count($res["styles"]); $i++){
            if(array_key_exists("css", $res["styles"][$i])) {
                $res["styles"][$i]["css"] = preg_replace("/\/\*.+?\*\/\n*/ui", "", $res["styles"][$i]["css"]);
            }
        }
        return $res;
        // phpcs:enable
