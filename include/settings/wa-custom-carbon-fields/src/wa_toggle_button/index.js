...
class WaToggleButton extends Component {
	...
	render() {
        ...
        return (
            <span class={classes("wa-toggle-button", {
                    ...
                    "wa-toggle-button--with-gutter" : checked && this.props.hasGutter
                })}>
                <input type="checkbox" className="wa-toggle-button__checkbox" onChange={(e) => this.props.onChange(this.id, e.target.checked)} checked={checked} id={this.id} name={name} />
                <label className="wa-toggle-button__label" for={this.id}>
                    <span>{label}</span>
                    {this.props.ownedByList && !this.props.lastInList ? <div className="wa-toggle-button__gutter"></div> : ""}
                </label>
            </span>
        );
	}
