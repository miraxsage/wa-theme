...
class WaSidebarsProfileBarItem extends Component {
  constructor(props) {
    super(props);
  }
  render() {
      ...
      >
        {(provided, snapshot) => (
          <div
            ...
            className={classes("wa-sidebars-profile-bar-item", {
              active: this.props.active,
              forbidden: this.props.forbidden,
              moving: this.props.moving,
            })}
            onClick={(e) =>
              !e.target.closest(".wa-sidebars-profile-bar-item-buttons") &&
              this.props.onEdit()
            }
          ...
          >
            <IconButton
              ...
              style={{
                display:
                  this.props.config.key == "common" ? "none" : "inline-block",
              }}
              inheritColor={true}
            />
...
class WaSidebarsProfileBar extends Component {
  constructor(props) {
    super(props);
    this.state = { forbiddenedToMoveItem: null, movingItem: null, filter: "" };
  ...
  render() {
    let items = this.props.config.filter(
      (c) =>
        !this.state.filter ||
        c.profile.toLowerCase().indexOf(this.state.filter.toLowerCase()) >= 0
    );
    return (
      <div className="wa-sidebars-profile-bar">
        <div className="header">
          ...
          <IconButton
            ...
            type="add"
            onClick={() => this.setState({ ...this.state, filter: "" })}
          />
          <IconButton
        ...
        >
          <Droppable droppableId="sidebars-profile-bar-droppable">
            {(provided, snapshot) => (
              <div
                className={classes("list", { moving: this.state.movingItem })}
                {...provided.droppableProps}
                ref={provided.innerRef}
              >
                {items.map((c, i) => (
                  <WaSidebarsProfileBarItem
                    key={c.key}
                    index={i}
                    forbidden={c.key == this.state.forbiddenedToMoveItem}
                    active={c.key == this.props.editingProfile}
                    moving={c.key == this.state.movingItem}
                    config={c}
                    onEdit={() => this.props.onEdit(c.key)}
                    onDelete={() =>
                      this.props.onChange(
                        this.props.config.filter((_c) => _c.key != c.key)
                      )
                    }
                  />
                ))}
        ...
        >
          {items.length > 0 ? (
            <>
              ...
              списка, обладают более высоким приоритетом и применяются первыми.
            </>
          ) : (
...
export default WaSidebarsProfileBar;
