...
class WaAdvancedPillList extends WaPersonalizedComponent {
	...
	constructor(props) {
		...
		for (let key of Object.keys(choices)) {
			...
			this.state.items.push({
				key, label, sortable, classes, checked, metaitem, status: null
			});
		}
	...
	changeVisibility(item_index){
		if(this.state.items[item_index].metaitem)
			return;
		let newItems = [...this.state.items];
		if(!this.nonPersonalizedMoment())
			newItems[item_index] = {...newItems[item_index], checked: !newItems[item_index].checked};
	...
	onDragEnd(result){
		...
		if(!this.checkItemPosValidity(item, result.source.index, result.destination.index, false))
			return;
		let newItems = [...this.state.items];
	...
	checkItemPosValidity(item, initpos, pos, setStatus = true){
		if(item.key.indexOf("similar_records") >= 0){
			item.status = null;
			let header_open = this.state.items.reduce((prev, curr, index) => curr.key.indexOf("header_open") >= 0 ? index : prev);
			let header_close = this.state.items.reduce((prev, curr, index) => curr.key.indexOf("header_close") >= 0 ? index : prev);
			if((pos > header_open && pos < header_close) || (initpos < header_open && pos == header_open) || (initpos > header_close && pos == header_close)){
				if(setStatus)
		...
		return (
			<div ref={this.rootRef} className={classes("wa-advanced-pill-list", this.getPersonalizedClass())}>
				<WaResetOptionsHeader label={field.label} {...this.resetProps} context={field.context}>
						{this.description ? <span className="wa-advanced-pill-list__description">{this.description}</span> : ""}
						<input type="hidden" id={this.props.id} name={this.props.name} value={this.getValueToSave(this.props.value)} />
									...
									>
										{this.state.items.map((item, index) => (
											<Draggable key={item.key} draggableId={item.key} index={index}>
												{(provided, snapshot) => {
													return (<div
															className={["wa-advanced-pill-list__item", item.checked ? "wa-advanced-pill-list__item_checked" : "",
																(item.status == "forbidden" ? " blocked" : ""),
																...item.classes.replace(",", " ").replace(/\s+/, " ").split(" ")].join(" ")}
															ref={provided.innerRef}
															{...provided.draggableProps}
															{...(item.metaitem ? {} : provided.dragHandleProps)}
															onClick={() => this.changeVisibility(index)}
														>
														{item.metaitem ? <div style={{display: "none"}} {...provided.dragHandleProps}></div> : ""}
														<span class={"dashicons" + (item.checked ? " dashicons-visibility" : " dashicons-hidden")} 
															style={{display: item.metaitem ? "none" : "inline-block"}}></span>
