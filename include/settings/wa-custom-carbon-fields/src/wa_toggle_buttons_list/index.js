...
class WaToggleButtonsList extends WaPersonalizedComponent {
    ...
    onChange(id, val) {
        ...
        if (this.props.field.mode == "radio" && !val) return;
        const key = id.replace(this.id + "_", "");
        this.buttons = this.buttons.map((b) => {
            if (b.key == key) return { ...b, act: val == true };
    ...
    onPersonalizedChanged(personalized, commonValue) {
        if (!personalized) this.props.onChange(this.id, commonValue);
    }
    ...
    render() {
        const {
            ...
            style,
            endCorners,
            field: { description, label, context },
        } = this.props;
        if (!value) return "Empty params";
        ...
        return (
            <div
                ...
                style={style}
                class={classes(
                    "wa-toggle-button-list",
                    this.getPersonalizedClass()
                )}
            >
                <WaResetOptionsHeader
                    collapse={withoutHeader}
                    label={label}
                    {...this.resetProps}
                    context={context}
                >
                    {description ? (
                        <em className="cf-field__help wa-toggle-buttons-list__description">
                            {description}
                        </em>
                    ) : (
                        ""
                    ...
                    <input
                        ...
                        id={this.id}
                        name={name}
                        value={this.getValueToSave(value)}
                    />
                </WaResetOptionsHeader>
            </div>
        );
