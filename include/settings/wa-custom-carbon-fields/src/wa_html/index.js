...
class WaHtml extends Component {
    ...
    componentDidMount() {
        ...
        let saveHeader = this.props.saveHeader ?? this.props.field.saveHeader ?? false;
        let saveDivider = this.props.saveDivider ?? this.props.field.saveDivider ?? false;
        ...
        if(!saveDivider && typeof jQuery !== "undefined")
            jQuery(this.rootRef.current).closest(".cf-field").next(".cf-field").css("border", "0px");
        if(this.props.field.mode == "category_container_description"){
            ...
            jQElement.detach();
            if(parent.is("td"))
                parent.prev().css({"vertical-align": "top", "padding": "30px 0px 0px 0px"}).append(jQElement);
            else{
	...
	render() {
        let { field: { mode } } = this.props;
        return (
            <div ref={this.rootRef} className={classes("wa-html", 
                {"wa-html--description cf-field__help": mode == "description",
                 "wa-html--container-description": mode == "category_container_description"})}>
