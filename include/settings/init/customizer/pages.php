...
init_customizer_container($customizer, 'page_additional__container', 'Расширенные настройки', 'wa_pages');
$cur_args = ['section' => 'wa_pages', 'container' => 'page_additional__container'];
    $customizer->add_setting("page_additional__redirect_descr", ["default" => ""]);
    $customizer->add_control(new WebArch_HTML_Control($customizer, "page_additional__redirect_descr",
    array_merge(
        $cur_args,
        [
            'content' => <<<CODE
            Следующие настройки для страниц (а также записей и архивов):<br/>
            ...
            осуществляются на в соответствующей <a href="/wp-admin/admin.php?page=wa-profiled-settings.php&target=pages">вкладке</a> раздела «Виджеты и профили»
    CODE
        ]))
    );
